...
export default function FaqSection() {
  const [openIndex, setOpenIndex] = useState<number | null>(1); // second FAQ opened by default
  ...
  return (
    <section className="py-16 px-4">
      <div className="w-full mx-auto flex flex-col items-center text-center">
        <Image
          src="/dragons/Nariko.svg" // Replace with your local image path
          alt="Cute dragon"
          width={154}
          height={177}
          className="mx-auto mb-6"
        />
        <h2 className="text-[45px] font-[500] text-[#984A2E] leading-[120%] tracking-[0.41px] mb-4">Frequently asked questions</h2>
        ...
        <div className="w-[80%] space-y-1 text-left ">
          {faqData.map((item, index) => (
            <div key={index} className={`rounded-xl overflow-hidden border-b border-[#0C1B4D1A]/10 ${openIndex === index ? 'bg-white/60 backdrop-blur-[40px] ' : 'hover:bg-white/60'} transition-all duration-300`}>
              <button
