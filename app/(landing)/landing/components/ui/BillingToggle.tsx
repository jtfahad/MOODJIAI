...
export const BillingToggle: React.FC<BillingToggleProps> = ({ isYearly, onToggle }) => {
  return (
    <div className="flex items-center justify-center gap-4 mb-10">
      ...
      >
        <div className={`w-14 h-7 rounded-full transition-colors duration-300 ${isYearly ? 'bg-white/50' : 'bg-white'}`}>
          <div className={`absolute top-[2px] left-[2px] h-6 w-6 rounded-full transition-transform duration-300 ${isYearly ? 'translate-x-full bg-black' : 'bg-black'}`} />
        </div>
      </button>
